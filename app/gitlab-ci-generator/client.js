...
const LANGUAGES = [
  { id: 'nodejs', label: 'Node.js', icon: '⬢', image: 'node:20-alpine', versions: ['22', '20', '18', '16'], lint: 'npm run lint', test: 'npm test', build: 'npm run build', install: 'npm ci', cache: 'node_modules/', lockfile: 'package-lock.json' },
  { id: 'python', label: 'Python', icon: '🐍', image: 'python:3.12-slim', versions: ['3.13', '3.12', '3.11', '3.10', '3.9'], lint: 'flake8 . && mypy .', test: 'pytest --cov', build: null, install: 'pip install -r requirements.txt', cache: '.pip-cache/', lockfile: 'requirements.txt' },
  { id: 'go', label: 'Go', icon: '🔵', image: 'golang:1.23-alpine', versions: ['1.23', '1.22', '1.21', '1.20'], lint: 'golangci-lint run', test: 'go test ./...', build: 'CGO_ENABLED=0 go build -ldflags="-s -w" -o app ./cmd/...', install: 'go mod download', cache: '/go/pkg/mod/', lockfile: 'go.sum' },
  { id: 'java', label: 'Java (Maven)', icon: '☕', image: 'maven:3.9-eclipse-temurin-21', versions: ['21', '17', '11'], lint: 'mvn checkstyle:check', test: 'mvn test', build: 'mvn package -DskipTests', install: null, cache: '.m2/repository/', lockfile: 'pom.xml' },
  { id: 'java-gradle', label: 'Java (Gradle)', icon: '☕', image: 'gradle:8-jdk21', versions: ['21', '17', '11'], lint: 'gradle checkstyleMain', test: 'gradle test', build: 'gradle build -x test', install: null, cache: '.gradle/', lockfile: 'build.gradle' },
  { id: 'rust', label: 'Rust', icon: '🦀', image: 'rust:1.80-slim', versions: ['1.80', '1.78', '1.75'], lint: 'cargo clippy -- -D warnings', test: 'cargo test', build: 'cargo build --release', install: null, cache: 'target/', lockfile: 'Cargo.lock' },
  { id: 'php', label: 'PHP', icon: '🐘', image: 'php:8.3-cli', versions: ['8.3', '8.2', '8.1'], lint: 'vendor/bin/phpstan analyse', test: 'vendor/bin/phpunit', build: null, install: 'composer install --no-dev --optimize-autoloader', cache: 'vendor/', lockfile: 'composer.lock' },
  { id: 'ruby', label: 'Ruby', icon: '💎', image: 'ruby:3.3-slim', versions: ['3.3', '3.2', '3.1'], lint: 'bundle exec rubocop', test: 'bundle exec rspec', build: null, install: 'bundle install', cache: 'vendor/ruby/', lockfile: 'Gemfile.lock' },
  { id: 'dotnet', label: '.NET', icon: '🔷', image: 'mcr.microsoft.com/dotnet/sdk:8.0', versions: ['9.0', '8.0', '7.0', '6.0'], lint: 'dotnet format --verify-no-changes', test: 'dotnet test', build: 'dotnet publish -c Release -o out', install: 'dotnet restore', cache: null, lockfile: null },
  { id: 'docker', label: 'Docker Only', icon: '🐳', image: 'docker:27', versions: [], lint: 'hadolint Dockerfile', test: null, build: null, install: null, cache: null, lockfile: null },
];
...
function generateYAML(config) {
  ...
  // Header
  comment('=================================================');
  comment(` GitLab CI/CD Pipeline — ${lang.label}`);
    ...
    <ToolLayout title="GitLab CI/CD Generator" description="Build a .gitlab-ci.yml pipeline config. Pick your stack, configure stages, download the file.">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SERVICES.map(s => (
                <Chip key={s.id} selected={config.services.includes(s.id)} onClick={() => toggleService(s.id)} icon={s.id === 'postgres' ? '🐘' : s.id === 'redis' ? '🔴' : s.id === 'mysql' ? '🐬' : '🍃'}>{s.label}</Chip>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              {config.deploy !== 'none' && (
                <>
                  <Toggle checked={config.deployStaging} onChange={v => set('deployStaging', v)} label="Staging → Production" desc="Deploy to staging on push, production on tags" />
        ...
        <div>
          ...
          {requiredVars.length > 0 && (
            <div className="mt-4 bg-dark-800/50 border border-dark-700 rounded-xl p-4">
              <div className="text-xs font-semibold text-amber-400 mb-2">⚠ Required CI/CD Variables</div>
          ...
          <div className="mt-4 flex items-center gap-2 overflow-x-auto pb-2">
            {[
              ...
              <div key={stage} className="flex items-center gap-2">
                ...
                {i < arr.length - 1 && <span className="text-dark-600">→</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-10 border-t border-dark-700 pt-6">
        ...
        <div className="text-xs text-dark-400 leading-relaxed space-y-2">
          ...
          <p><strong className="text-dark-300">Stages</strong> define the order of execution. A common pattern is lint → test → build → deploy. If any job in a stage fails, the next stage won&apos;t run (unless configured with <code className="text-dark-300">allow_failure: true</code>). <strong className="text-dark-300">Services</strong> like PostgreSQL or Redis can be attached to jobs for integration testing — they run as Docker containers alongside your job.</p>
          <p><strong className="text-dark-300">Caching</strong> speeds up pipelines by preserving directories like <code className="text-dark-300">node_modules</code> or <code className="text-dark-300">.pip-cache</code> between runs. <strong className="text-dark-300">Artifacts</strong> pass files between stages — for example, the build stage produces a binary that the deploy stage uses. <strong className="text-dark-300">Rules</strong> control when jobs run: deploy only on the main branch, or require manual approval for production.</p>
          <p>This generator creates production-ready pipelines following GitLab CI/CD best practices. It supports Docker-in-Docker builds, multi-environment deployments (staging → production), coverage reporting in merge requests, and JUnit test report integration. All configuration runs in your browser — no data is sent to any server.</p>

          <p className="mt-3"><strong className="text-dark-300">Need a complete walkthrough?</strong> Read our <a href="/blog/gitlab-ci-cd-guide/" className="text-brand-400 hover:underline">GitLab CI/CD Pipeline Guide 2026</a> — pipeline structure, Node.js/Python/Go examples, Docker builds, Kubernetes deployments, and caching strategies.</p>
        </div>
      </section>
    </ToolLayout>
  );
